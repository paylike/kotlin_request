...
class RequestTest {
    ...
    fun paylike_requester_form() {
        ...
        val opts = RequestOptions(form = true, formFields = mapOf("foo" to "bar"))
        val requester = PaylikeRequester()
        runBlocking {
            val response = requester.request(endpoint = "http://localhost:9000", opts = opts)
    ...
    fun paylike_requester_auto_retry() {
        ...
        val app: HttpHandler =
            fun(_: Request): Response {
                ...
                retried = true
                return Response(Status.TOO_MANY_REQUESTS).headers(listOf("retry-after" to "1"))
            }
        val jettyServer = app.asServer(Jetty(9000)).start()
    ...
    fun paylike_requester_rate_limit() {
        ...
        assertThrows(RateLimitException::class.java) {
            runBlocking { requester.request(endpoint = "http://localhost:9000", opts = opts) }
        }
        jettyServer.close()
    }
                    ...
                    .body(
                        Body(
                            buildJsonObject {
                                    put("message", "foo")
                                    put("code", "FOO")
                                    put("errors", buildJsonArray { add("bar") })
                                }
                                .toString()
                        )
                    )
    ...
    fun paylike_requester_server_error() {
        val app: HttpHandler =
            fun(_: Request): Response {
                return Response(Status.INTERNAL_SERVER_ERROR).headers(listOf("foo" to "bar"))
            }
        val jettyServer = app.asServer(Jetty(9000)).start()
        val opts = RequestOptions()
        val requester = PaylikeRequester()
    ...
    @Test
    fun request_options() {
        assertThrows(VersionException::class.java) { RequestOptions(version = 0) }
    }
}
